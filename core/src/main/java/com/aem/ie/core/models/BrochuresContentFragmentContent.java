...
public class BrochuresContentFragmentContent {
	private static final Logger log = LoggerFactory.getLogger(BrochuresContentFragmentContent.class);
	@Inject
	...
	private String cfBrochureImagePath = null;
	InputStream inTitle;
	InputStream inDescription;
	InputStream inPdflink;
	InputStream inBrochureImagePath;
	...
	public Map<String, String> getBrochuresMyHashMap() {
		return myHashMap;
	}
				...
				if (cfNode != null) {
					if (cfNode.hasProperty("title") && cfNode.hasProperty("description@ContentType")) {
						if (cfNode.getProperty("description@ContentType").getString().equals("text/html")) {
							...
							inPdflink = cfNode.getProperty("pdfimagepath").getBinary().getStream();
							cfPdflink = IOUtils.toString(inPdflink, "UTF-8");
							inBrochureImagePath = cfNode.getProperty("brochureImagePath").getBinary().getStream();
							cfBrochureImagePath = IOUtils.toString(inBrochureImagePath, "UTF-8");
							...
							myHashMap2.put(cfTitle, cfBrochureImagePath);
						}
					} else {
