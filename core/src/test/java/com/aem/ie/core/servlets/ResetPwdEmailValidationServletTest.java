...
class ResetPwdEmailValidationServletTest {
    AemContext aemContext =  new AemContext(ResourceResolverType.JCR_MOCK);

    @Mock
    IECTResetPasswordEmailValidation IECTResetPasswordEmailValidation;
    private IECTResetPasswordEmailValidation mockIECTResetPasswordEmailValidation;
    @InjectMocks
    ResetPwdEmailValidationServlet MockResetPwdEmailValidationServlet;

    private Map<String, String> configProps = new HashMap<String, String>();
    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);
        configProps.put("getResetPasswordCTURL", "https://apim-iedtecomm-dev01.azure-api.net/cart-direct/fairview/delete/");
        ...
        mockIECTResetPasswordEmailValidation = aemContext.registerInjectActivateService(new IECTResetPasswordEmailImpl(), configProps);
    }
    @Test()
    void doPost() throws ServletException, IOException,NullPointerException {
        ...
        NullPointerException thrown = Assertions.assertThrows(NullPointerException.class, () -> {
            ...
            String MockClearCartResponse = mockIECTResetPasswordEmailValidation.getResetPasswordEmailCTUrl(email, bearerToken);
            when(IECTResetPasswordEmailValidation.getResetPasswordEmailCTUrl(email, bearerToken)).thenReturn(MockClearCartResponse);
            MockSlingHttpServletResponse response = aemContext.response();
            request.addRequestParameter("bearertoken", bearerToken);
            request.addRequestParameter("email", email);
            MockResetPwdEmailValidationServlet.doPost(request, response);
        });
    }
