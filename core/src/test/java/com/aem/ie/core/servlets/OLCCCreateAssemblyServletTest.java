...
class OLCCCreateAssemblyServletTest {
    ...
    @InjectMocks
    OLCCCreateAssemblyServlet MockOLCCCreateAssemblyServlet;
    ...
    void setUp() {
        ...
        configProps.put("getCreateUrl", "https://apim-iedtecomm-dev01.azure-api.net/olcc/fairview/create-olcc");
    ...
    void doPost() throws ServletException, IOException {
        ...
        String MockCreateAssemblyResponse = mockSampleOSGIService.createAssembly(createAssemblyInputRequest(),bearerToken);
        when(MOCKOLCCModuleService.createAssembly(Mockito.anyString(),Mockito.anyString())).thenReturn(MockCreateAssemblyResponse);
        MockSlingHttpServletRequest request = aemContext.request();
        MockSlingHttpServletResponse response = aemContext.response();
        request.addRequestParameter("bearertoken",bearerToken);
        request.addRequestParameter("jsonData",createAssemblyInputRequest());
        MockOLCCCreateAssemblyServlet.doPost(request,response);
        //assertEquals(true,response.getOutputAsString().contains("id"));
    }

    private String createAssemblyInputRequest() {
        return "{\n" +
                "    \"caCon1\": \"SC7016\",\n" +
                "    \"caCon2\": \"SC7016\",\n" +
                "    \"caCoax\": \"FM-SR141TB\",\n" +
